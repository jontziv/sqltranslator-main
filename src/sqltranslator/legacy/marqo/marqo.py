...
class Marqo_VectorStore(VannaBase):
    def __init__(self, config=None):
        ...
        for index in ["sqltranslator-sql", "sqltranslator-ddl", "sqltranslator-doc"]:
            try:
    ...
    def add_question_sql(self, question: str, sql: str, **kwargs) -> str:
        ...
        self.mq.index("sqltranslator-sql").add_documents(
            [question_sql_dict],
    ...
    def add_ddl(self, ddl: str, **kwargs) -> str:
        ...
        self.mq.index("sqltranslator-ddl").add_documents(
            [ddl_dict],
            tensor_fields=["ddl"],
        )
        return id
    ...
    def add_documentation(self, documentation: str, **kwargs) -> str:
        ...
        self.mq.index("sqltranslator-doc").add_documents(
            [doc_dict],
            tensor_fields=["doc"],
        )
        return id
    ...
    def get_training_data(self, **kwargs) -> pd.DataFrame:
        ...
        for hit in self.mq.index("sqltranslator-doc").search("", limit=1000)["hits"]:
            data.append(
                {
                    "id": hit["_id"],
        ...
        for hit in self.mq.index("sqltranslator-ddl").search("", limit=1000)["hits"]:
            data.append(
                {
                    "id": hit["_id"],
        ...
        for hit in self.mq.index("sqltranslator-sql").search("", limit=1000)["hits"]:
            data.append(
                {
                    "id": hit["_id"],
    ...
    def remove_training_data(self, id: str, **kwargs) -> bool:
        if id.endswith("-sql"):
            self.mq.index("sqltranslator-sql").delete_documents(ids=[id])
            return True
        elif id.endswith("-ddl"):
            self.mq.index("sqltranslator-ddl").delete_documents(ids=[id])
            return True
        elif id.endswith("-doc"):
            self.mq.index("sqltranslator-doc").delete_documents(ids=[id])
            return True
        else:
            return False
    ...
    def get_similar_question_sql(self, question: str, **kwargs) -> list:
        return Marqo_VectorStore._extract_documents(
            self.mq.index("sqltranslator-sql").search(question)
        )

    def get_related_ddl(self, question: str, **kwargs) -> list:
        return Marqo_VectorStore._extract_documents(
            self.mq.index("sqltranslator-ddl").search(question)
        )

    def get_related_documentation(self, question: str, **kwargs) -> list:
        return Marqo_VectorStore._extract_documents(
            self.mq.index("sqltranslator-doc").search(question)
        )
