"""
HTML templates for SQLTranslator servers.
"""

from typing import Optional


def get_sqltranslator_component_script(
    dev_mode: bool = False,
    static_path: str = "/static",
    cdn_url: str = "https://img.vanna.ai/sqltranslator-components.js",
) -> str:
    """Get the script tag for loading SQLTranslator web components.
    ...
    if dev_mode:
        return (
            f'<script type="module" src="{static_path}/sqltranslator-components.js"></script>'
        )
    else:
...
def get_index_html(
    ...
    """
    component_script = get_sqltranslator_component_script(dev_mode, static_path, cdn_url)
...
<head>
    ...
    <title>SQLTranslator Chat</title>
...
<body>
    <div class="max-w-6xl mx-auto p-5">
        ...
        <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-vanna-navy mb-2 font-serif">SQLTranslator</h1>
            <p class="text-lg font-mono font-bold text-vanna-teal mb-4">DATA-FIRST AGENTS</p>
            <p class="text-slate-600 mb-4">Interactive AI Assistant powered by SQLTranslator Framework</p>
        ...
        <div id="chatSections" class="hidden">
            <div class="bg-white rounded-xl shadow-lg h-[600px] overflow-hidden border border-vanna-teal/30">
                <sqltranslator-chat
                    ...
                    sse-endpoint="{api_base_url}/api/sqltranslator/v2/chat_sse"
                    ws-endpoint="{api_base_url}/api/sqltranslator/v2/chat_websocket"
                    poll-endpoint="{api_base_url}/api/sqltranslator/v2/chat_poll">
                </sqltranslator-chat>
            </div>
            ...
            <div class="mt-8 p-5 bg-white rounded-lg shadow border border-vanna-teal/30">
                ...
                <ul class="space-y-2">
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
                        <span class="font-bold text-vanna-teal mr-2">POST</span>{api_base_url}/api/sqltranslator/v2/chat_sse - Server-Sent Events streaming
                    </li>
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
                        <span class="font-bold text-vanna-teal mr-2">WS</span>{api_base_url}/api/sqltranslator/v2/chat_websocket - WebSocket real-time chat
                    </li>
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
                        <span class="font-bold text-vanna-teal mr-2">POST</span>{api_base_url}/api/sqltranslator/v2/chat_poll - Request/response polling
                    </li>
                    <li class="p-2 bg-vanna-cream/50 rounded font-mono text-sm">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            const email = getCookie('sqltranslator_email');
            ...
            loginButton.addEventListener('click', () => {{
                ...
                setCookie('sqltranslator_email', email);
                loginContainer.classList.add('hidden');
                loggedInStatus.classList.remove('hidden');
                chatSections.classList.remove('hidden');
                loggedInEmail.textContent = email;
            ...
            logoutButton.addEventListener('click', () => {{
                deleteCookie('sqltranslator_email');
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            const sqlTranslatorChat = document.querySelector('sqltranslator-chat');

            if (sqlTranslatorChat) {{
                // Add artifact event listener to demonstrate external rendering
                sqlTranslatorChat.addEventListener('artifact-opened', (event) => {{
                    ...
                    setTimeout(() => {{
                        ...
                        if (newWindow) {{
                            ...
                            newWindow.document.title = title || 'SQLTranslator Artifact';
        ...
        if (!customElements.get('sqltranslator-chat')) {{
            setTimeout(() => {{
                if (!customElements.get('sqltranslator-chat')) {{
                    document.querySelector('sqltranslator-chat').innerHTML = `
                        <div class="p-10 text-center text-gray-600">
                            <h3 class="text-xl font-semibold mb-2">SQLTranslator Chat Component</h3>
