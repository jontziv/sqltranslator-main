...
const addMockSQLTranslatorChat = (container: HTMLElement) => {
  // Create a mock sqltranslator-chat element with sendMessage method
  const mockSQLTranslatorChat = document.createElement('div');
  mockSQLTranslatorChat.setAttribute('id', 'mock-sqltranslator-chat');

  // Store the original querySelector
  const originalQuerySelector = document.querySelector.bind(document);

  // Override querySelector to return our mock when looking for sqltranslator-chat
  document.querySelector = function(selector: string) {
    if (selector === 'sqltranslator-chat') {
      return mockSQLTranslatorChat as any;
    }
    return originalQuerySelector(selector);
  } as any;

  // Add sendMessage method that logs to console and shows in UI
  (mockSQLTranslatorChat as any).sendMessage = (message: string) => {
  ...
  container.appendChild(mockSQLTranslatorChat);
  return mockSQLTranslatorChat;
};
...
export const SingleButtons: Story = {
  render: () => {
    ...
    addMockSQLTranslatorChat(container);
