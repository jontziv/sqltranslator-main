      ...
      actionButtons.forEach((button, index) => {
        ...
        if (action && action.action) {
          ...
          button.addEventListener('click', async () => {
            ...
            // Find sqltranslator-chat component and send message
            const vannaChat = document.querySelector('sqltranslator-chat') as any;
            ...
            if (vannaChat && typeof vannaChat.sendMessage === 'function') {
              ...
              console.warn('⚠️ sqltranslator-chat component not found or sendMessage not available');
              button.disabled = false;
              button.classList.remove('button-transitioning', 'button-clicked');
            }
          });
  ...
  render(component: RichComponent): HTMLElement {
    ...
    if (action && !disabled && !loading) {
      button.addEventListener('click', async () => {
        ...
        // Find sqltranslator-chat component and send message with button action
        const vannaChat = document.querySelector('sqltranslator-chat') as any;
        console.log('   Found sqltranslator-chat:', !!vannaChat);
        ...
        if (vannaChat && typeof vannaChat.sendMessage === 'function') {
          ...
          console.error('   ✗ sqltranslator-chat not found or sendMessage not available');
          // Restore button state if it wasn't originally disabled
          if (!disabled) {
            button.disabled = false;
          }
...
export class ChartComponentRenderer extends BaseComponentRenderer {
  render(component: RichComponent): HTMLElement {
    ...
    if (plotlyData && Array.isArray(plotlyData) && layout) {
      ...
      const vannaChat = document.querySelector('sqltranslator-chat');
      if (vannaChat) {
...
export class ArtifactComponentRenderer extends BaseComponentRenderer {
  ...
  private fireArtifactOpenedEvent(component: RichComponent, trigger: 'created' | 'user-action', container: HTMLElement): boolean {
    ...
    // Fire the event from the container element (should bubble up to sqltranslator-chat)
    container.dispatchEvent(event);

    // Also dispatch directly on the sqltranslator-chat element as backup
    const vannaChat = container.closest('sqltranslator-chat');
    if (vannaChat) {
      console.log('📡 Also dispatching on sqltranslator-chat element');
...
export class UserMessageComponentRenderer extends BaseComponentRenderer {
  render(component: RichComponent): HTMLElement {
    const messageEl = document.createElement('sqltranslator-message');
    messageEl.setAttribute('theme', 'light'); // Could be made dynamic
    messageEl.dataset.componentId = component.id;
    
    // Set properties for sqltranslator-message
...
export class ComponentRegistry {
  ...
  private renderWebComponent(tagName: string, component: RichComponent): HTMLElement {
    ...
    // Set theme to match the parent SQLTranslatorChat theme
  ...
  private getCurrentTheme(): string {
    // Try to get theme from the parent SQLTranslatorChat component
    const vannaChat = document.querySelector('sqltranslator-chat');
    if (vannaChat) {
...
export class ComponentManager {
  ...
  private triggerScroll(): void {
    // Find the parent sqltranslator-chat component and trigger its scroll method
    const vannaChat = document.querySelector('sqltranslator-chat') as any;
  ...
  private updateStatusBar(component: RichComponent): void {
    ...
    // Look for sqltranslator-chat and search within its shadow root
    const vannaChat = document.querySelector('sqltranslator-chat') as any;
