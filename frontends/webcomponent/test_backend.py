#!/usr/bin/env python3
"""
Comprehensive test backend for sqltranslator-webcomponent validation.
...
# Add sqltranslator to path
sys.path.insert(0, "../sqltranslator/src")

from sqltranslator.core.rich_component import RichComponent, ComponentLifecycle
from sqltranslator.components.rich import (
    RichTextComponent,
    StatusCardComponent,
...
from sqltranslator.servers.base.models import ChatStreamChunk

# Request/Response models
class ChatRequest(BaseModel):
    """Chat request matching sqltranslator API."""
    message: str
...
Watch the sidebar checklist as components render! ➡️""",
    ...
    # NOTE: Table, Container, and CodeBlock components are defined in sqltranslator Python package
    # but NOT supported by the webcomponent (no renderers). Skipping these tests.
    # These are candidates for removal from the sqltranslator package.
...
# FastAPI app
app = FastAPI(title="SQLTranslator Webcomponent Test Backend")
...
@app.post("/api/sqltranslator/v2/chat_sse")
...
async def root():
    ...
    return {
        "message": "SQLTranslator Webcomponent Test Backend",
        "mode": test_state["mode"],
        "endpoints": {
            "chat": "POST /api/sqltranslator/v2/chat_sse",
            "health": "GET /health",
        },
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test backend for sqltranslator-webcomponent")
    parser.add_argument(
        "--mode",
